...
Generation date: 2026-05-06 16:33:24
...
state_t do_init(T &data) {
  state_t next_state = cncpp::STATE_IDLE; // by default to idle
  try {
    data.program = make_unique<cncpp::Program>(data.program_file, data.machine.get());
    cerr << cncpp::log_tag(cncpp::LogType::MESSAGE) << " Program loaded successfully: "<<endl
         << *data.program << endl;
    // manda tutto a puttane
    next_state = cncpp::STATE_LOAD_BLOCK;
  } catch (const std::exception &e) {
    cerr << cncpp::log_tag(cncpp::LogType::ERROR) << " Failed to load program: "
         << e.what() << endl;
    next_state = cncpp::STATE_STOP; // if loading fails, go to stop state
  }
...
state_t do_idle(T &data) {
  ...
  // The reader may take time
  data.timer->stop();
  char key = keystroker::read_key();
  switch(key) {
    case ' ':
      next_state = cncpp::STATE_LOAD_BLOCK;
      data.program->rewind(); // reset program iterator to the beginning
      break;
    case 'r':
    case 'R':
      try {
        data.program->load(data.program_file, false); // reload program from file, resetting the current list
        cerr << cncpp::log_tag(cncpp::LogType::MESSAGE) 
             << " Program reloaded successfully: "<<endl
             << *data.program << endl;
      } catch (const std::exception &e) {
    ...
    case 'Q':
      ...
      break;
    default:
      cerr << cncpp::log_tag(cncpp::LogType::WARNING) 
           << " Wrong selection, pick another key."<< endl;
      break;
  }
  data.timer->start(); // restart timer after reading input
...
state_t do_load_block(T &data) {
  ...
  if (data.program->done()) {
    return cncpp::STATE_IDLE; // if program is done, go back to idle state
  }
  ...
  switch (b.type()) {
    ...
    case Block::BlockType::CCWA:
      ...
      break;
    default:
      next_state = cncpp::STATE_IDLE; // if block type is unrecognized, skip it and load the next one
      break;
  }

  data.t_tot += data.machine->tq(); // update total time with the expected block time, to keep track of the overall execution time
...
state_t do_rapid_motion(T &data) {
  ...
  cerr << fg::cyan << "Rapid motion block: " << fg::green 
       << b << style::reset;

  data.t_tot += data.machine->tq(); // update block time with the expected block time, to keep track of the current block execution time
  return next_state;
}
...
state_t do_interp_motion(T &data) {
  ...
  try {
    cout << fmt::format("{:0>3d}, {:0>2d}, {:.3f}, {:.3f}, {:.3f}, {:.3f},{:.1f}, {:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}", b.n(), static_cast<int>(b.type()), data.t_tot, data.t_blk, lambda, lambda * b.length(), speed, b.profile()->current_acc(), tgt.x(), tgt.y(), tgt.z(), p.x(),p.y(),p.z());
  } catch (const bad_optional_access &e) {
    cout << fmt::format("{:0>3d}, {:0>2d}, {:.3f}, {:.3f}, {:.3f}, {:.3f},,{:.1f}, {:.3f},{:.3f},{:.3f},{:.3f},{:.>3},{:.>3},{:.>3}", b.n(), static_cast<int>(b.type()), data.t_tot, data.t_blk, lambda, lambda * b.length(), speed, b.profile()->current_acc(), tgt.x(), tgt.y(), tgt.z(), "-", "-", "-");
  }

  if (data.t_blk > b.dt() + tq/10.0)
    next_state = cncpp::STATE_LOAD_BLOCK; // if block time is exceeded, load the next block
...
state_t do_stop(T &data) {
  state_t next_state = cncpp::NO_CHANGE;
  cerr << fg::red << style::bold << "[STOP] " << style::reset << endl;

  // qualcosa
  signal(SIGINT, SIG_DFL); // restore default behavior for SIGINT to allow emergency stop by user
...
void reset(T &data) {
  auto &p = data.program;
  p->rewind(); // reset program iterator to the beginning
  data.t_tot = 0.0; // reset total time
  data.t_blk = 0.0; // reset block time
  cout << "n,type,t_tot,t_blk,lambda,s,feedrate,acc,x_n,y_n,z_n,x,y,z" << endl; // print csv header for logging
}
...
void begin_rapid(T &data) {
  data.t_blk = 0.0; // reset block time at the beginning of a new rapid motion block
}
...
void end_rapid(T &data) {
  data.t_blk = 0.0; // reset block time at the end of a rapid motion block
}
