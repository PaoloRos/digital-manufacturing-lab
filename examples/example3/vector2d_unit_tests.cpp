...
int main() {
...

  {
    ...
    failures += !require(starts_with(v.label(), "vec_"),
                         "default constructor auto-label starts with vec_");
  }

  {
    ...
    failures += !require(starts_with(v.label(), "vec_"),
                         "(x,y) constructor auto-label starts with vec_");
...

  {
    ...
    const Vector2D b(3.0, 4.0);
    failures += !require(approx_equal(a.distance_to(b), 5.0),
                         "distance_to() computes Euclidean distance");
    failures += !require(approx_equal(a.distance_to(b), b.distance_to(a)),
                         "distance_to() is symmetric");
  }
